 ...
 */
class FqPieQueueDiscTestItem : public QueueDiscItem
{
public:
  /**
   * Constructor
   ...
   */
  FqPieQueueDiscTestItem (Ptr<Packet> p, const Address & addr);
  virtual ~FqPieQueueDiscTestItem ();
  virtual void AddHeader (void);
  virtual bool Mark (void);

private:
  FqPieQueueDiscTestItem ();
  /**
   * \brief Copy constructor
   * Disable default implementation to avoid misuse
   */
  FqPieQueueDiscTestItem (const FqPieQueueDiscTestItem &);
  /**
   * \brief Assignment operator
   * \return this object
   * Disable default implementation to avoid misuse
   */
  FqPieQueueDiscTestItem &operator = (const FqPieQueueDiscTestItem &);
};

FqPieQueueDiscTestItem::FqPieQueueDiscTestItem (Ptr<Packet> p, const Address & addr)
  : QueueDiscItem (p, addr, 0)
{
}

FqPieQueueDiscTestItem::~FqPieQueueDiscTestItem ()
{
}

void
FqPieQueueDiscTestItem::AddHeader (void)
{
}

bool
FqPieQueueDiscTestItem::Mark (void)
{
  return false;
}
 ...
 */
class FqPieQueueDiscTestCase : public TestCase
{
public:
  FqPieQueueDiscTestCase ();
...
private:
   ...
   */
  void Enqueue (Ptr<FqPieQueueDisc> queue, uint32_t size, uint32_t nPkt);
  /**
   ...
   * \param nPkt the number of packets
   */
  void EnqueueWithDelay (Ptr<FqPieQueueDisc> queue, uint32_t size, uint32_t nPkt);
  /**
   * Dequeue function
   * \param queue the queue disc
   * \param nPkt the number of packets
...
FqPieQueueDiscTestCase::FqPieQueueDiscTestCase ()
  : TestCase ("Sanity check on the pie queue disc implementation")
{
}

void
FqPieQueueDiscTestCase::RunPieTest (QueueSizeUnit mode)
{
  uint32_t pktSize = 0;
  ...
  uint32_t qSize = 300;
  Ptr<FqPieQueueDisc> queue = CreateObject<FqPieQueueDisc> ();
  ...
  queue->Enqueue (Create<FqPieQueueDiscTestItem> (p1, dest));
  NS_TEST_EXPECT_MSG_EQ (queue->GetCurrentSize ().GetValue (), 1 * modeSize, "There should be one packet in there");
  queue->Enqueue (Create<FqPieQueueDiscTestItem> (p2, dest));
  NS_TEST_EXPECT_MSG_EQ (queue->GetCurrentSize ().GetValue (), 2 * modeSize, "There should be two packets in there");
  queue->Enqueue (Create<FqPieQueueDiscTestItem> (p3, dest));
  queue->Enqueue (Create<FqPieQueueDiscTestItem> (p4, dest));
  queue->Enqueue (Create<FqPieQueueDiscTestItem> (p5, dest));
  queue->Enqueue (Create<FqPieQueueDiscTestItem> (p6, dest));
  queue->Enqueue (Create<FqPieQueueDiscTestItem> (p7, dest));
  queue->Enqueue (Create<FqPieQueueDiscTestItem> (p8, dest));
  ...
  queue = CreateObject<FqPieQueueDisc> ();
  ...
  uint32_t test2 = st.GetNDroppedPackets (FqPieQueueDisc::UNFORCED_DROP);
  NS_TEST_EXPECT_MSG_NE (test2, 0, "There should be some unforced drops");
  NS_TEST_EXPECT_MSG_EQ (st.GetNDroppedPackets (FqPieQueueDisc::FORCED_DROP), 0, "There should be zero forced drops");
  ...
  Simulator::Run ();
  st = queue->GetStats ();
  uint32_t test3 = st.GetNDroppedPackets (FqPieQueueDisc::UNFORCED_DROP);
  ...
  Simulator::Run ();
  st = queue->GetStats ();
  uint32_t test4 = st.GetNDroppedPackets (FqPieQueueDisc::UNFORCED_DROP);
  ...
  Simulator::Run ();
  st = queue->GetStats ();
  uint32_t test5 = st.GetNDroppedPackets (FqPieQueueDisc::UNFORCED_DROP);
  NS_TEST_EXPECT_MSG_LT (test5, test4, "Test 5 should have less unforced drops than test 4");
  NS_TEST_EXPECT_MSG_EQ (st.GetNDroppedPackets (FqPieQueueDisc::FORCED_DROP), 0, "There should be zero forced drops");
}

void
FqPieQueueDiscTestCase::Enqueue (Ptr<FqPieQueueDisc> queue, uint32_t size, uint32_t nPkt)
{
  Address dest;
  for (uint32_t i = 0; i < nPkt; i++)
    {
      queue->Enqueue (Create<FqPieQueueDiscTestItem> (Create<Packet> (size), dest));
    }
}

void
FqPieQueueDiscTestCase::EnqueueWithDelay (Ptr<FqPieQueueDisc> queue, uint32_t size, uint32_t nPkt)
{
  Address dest;
  double delay = 0.01;  // enqueue packets with delay
  for (uint32_t i = 0; i < nPkt; i++)
    {
      Simulator::Schedule (Time (Seconds ((i + 1) * delay)), &FqPieQueueDiscTestCase::Enqueue, this, queue, size, 1);
    }
}

void
FqPieQueueDiscTestCase::Dequeue (Ptr<FqPieQueueDisc> queue, uint32_t nPkt)
{
  for (uint32_t i = 0; i < nPkt; i++)
    {
...
void
FqPieQueueDiscTestCase::DequeueWithDelay (Ptr<FqPieQueueDisc> queue, double delay, uint32_t nPkt)
{
  for (uint32_t i = 0; i < nPkt; i++)
    {
      Simulator::Schedule (Time (Seconds ((i + 1) * delay)), &FqPieQueueDiscTestCase::Dequeue, this, queue, 1);
    }
}

void
FqPieQueueDiscTestCase::DoRun (void)
{
 ...
 */
static class FqPieQueueDiscTestSuite : public TestSuite
{
public:
  FqPieQueueDiscTestSuite ()
    : TestSuite ("pie-queue-disc", UNIT)
  {
    AddTestCase (new FqPieQueueDiscTestCase (), TestCase::QUICK);
  }
} g_FqPieQueueTestSuite; ///< the test suite
