...
parser.add_argument('--packetSize', type=float)
...
os.system ("tshark -r " + file_name +" -T fields -e frame.time_relative -e frame.len -e ip.dst -e tcp.dstport -e udp.dstport> " + file_name[0:file_name.rindex('.')] + ".csv")
...
print(packetSize, " is the packet size")

with open(file_name[0:file_name.rindex('.')] + '.csv') as fq:
    fp = fq.readlines()
    for line in fp:
        # print(line.split('\t')[3].strip (), line.split('\t')[2].strip (), float(line.split('\t')[0]))
        if line.split('\t')[3].strip () != '' :
            print(int (line.split('\t')[3].strip ()), str(line.split('\t')[2]).strip ())
            if (destIp == None or str(line.split('\t')[2]).strip () == destIp) and (float(line.split('\t')[1]) == packetSize or packetSize == None) and float(line.split('\t')[0]) >= startTime and (destPort == None or destPort == int (line.split('\t')[3].strip ())):
